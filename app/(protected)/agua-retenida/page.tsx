    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-600 dark:text-slate-300 mt-1">9 CFR 441 · Protocolo de Control de Agua Retenida en Carnes</p>
        </div>
        <Link
      ...
      {tableExists && (
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { label: "Pruebas (90d)",      value: total,                                               icon: FlaskConical, color: "text-cyan-600",   bg: "bg-white dark:bg-[#111827]"       },
              { label: "Fuera de límite",    value: fails,                                               icon: XCircle,     color: fails > 0 ? "text-red-600" : "text-slate-600 dark:text-slate-300", bg: fails > 0 ? "bg-white dark:bg-[#111827]" : "bg-slate-100 dark:bg-slate-800" },
              { label: "Producto retenido",  value: onHold,                                             icon: AlertTriangle,color: onHold > 0 ? "text-amber-600" : "text-slate-600 dark:text-slate-300", bg: onHold > 0 ? "bg-white dark:bg-[#111827]" : "bg-slate-100 dark:bg-slate-800" },
          ...
          <div className="bg-white dark:bg-[#111827] border border-cyan-200 dark:border-cyan-800 rounded-xl px-5 py-4">
                    ...
                    {logs.map((log) => {
                      ...
                      return (
                        <tr key={log.id} className={`hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors ${overLimit ? "bg-red-50/30 dark:bg-red-900/10" : ""}`}>
                          ...
                          <td className="px-4 py-3 text-slate-600 dark:text-slate-300 text-xs font-mono">{log.lot_number || "—"}</td>
                          <td className="px-4 py-3 text-xs text-slate-600 dark:text-slate-300">{PRODUCT_TYPE[log.product_type] ?? log.product_type}</td>
                          <td className="px-4 py-3 text-xs text-slate-600 dark:text-slate-300">{CHILLING[log.chilling_method] ?? log.chilling_method}</td>
                          ...
                          <td className="px-4 py-3 text-xs text-slate-600 dark:text-slate-300 tabular-nums">{limit}%</td>
