    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-600 dark:text-slate-300 mt-1">9 CFR 417.4 · Verificación y aprobación de proveedores</p>
        </div>
        <Link
      ...
      {tableExists && (
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { label: "Total proveedores", value: suppliers.length, icon: Building2,    color: "text-teal-600",   bg: "bg-white dark:bg-[#111827]"       },
              { label: "Aprobados",         value: totalApproved,    icon: ShieldCheck,  color: "text-green-600",  bg: "bg-white dark:bg-[#111827]"     },
              { label: "Riesgo alto",       value: highRisk,         icon: AlertTriangle,color: highRisk > 0 ? "text-red-600" : "text-slate-600 dark:text-slate-300", bg: highRisk > 0 ? "bg-white dark:bg-[#111827]" : "bg-slate-100 dark:bg-slate-800" },
              { label: "Pendientes revisión",value: pendingReview,   icon: Clock,        color: pendingReview > 0 ? "text-amber-600" : "text-slate-600 dark:text-slate-300", bg: pendingReview > 0 ? "bg-white dark:bg-[#111827]" : "bg-slate-100 dark:bg-slate-800" },
            ].map((k) => (
          ...
          {/* Supplier cards */}
          {suppliers.length === 0 ? (
            <div className="bg-white dark:bg-[#111827] rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm flex flex-col items-center justify-center py-16 px-4">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 text-center max-w-xs mb-6">
                Registra tus proveedores aprobados y lleva control de verificaciones, riesgos y vencimientos según 9 CFR 417.4.
              </p>
              <Link href="/proveedores/nuevo" className="inline-flex items-center gap-2 px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-semibold rounded-lg transition-colors shadow-sm">
                <Plus className="w-4 h-4" />
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        ...
                        {s.est_number && (
                          <p className="text-xs text-slate-600 dark:text-slate-300 mt-0.5">EST {s.est_number}</p>
                        )}
                      </div>
                    ...
                    {isExpiringSoon && (
                      <div className="flex items-center gap-1.5 text-xs text-slate-900 dark:text-slate-100 bg-white dark:bg-[#111827] px-2.5 py-1.5 rounded-lg">
                  ...
                  return (
                    <div key={s.id} className="px-5 py-3 flex items-center justify-between gap-4">
                      <div>
                        ...
                        <p className="text-xs text-slate-600 dark:text-slate-300">{COMPANY_TYPE[s.company_type]}</p>
                      </div>
                      <div className="text-right flex-shrink-0">
                        <p className="text-sm font-bold text-amber-600">{fmtDate(s.approval_expiry)}</p>
                        <p className="text-xs text-slate-600 dark:text-slate-300">{daysLeft}d restantes</p>
                      </div>
                    </div>
                  )
                })}
