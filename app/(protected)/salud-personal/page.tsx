    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-600 dark:text-slate-300 mt-1">9 CFR 416.8 · Control de salud del personal de proceso</p>
        </div>
        <Link
          ...
          {recentExclusions.map((d) => (
            <div key={d.id} className="bg-white dark:bg-red-900/10 rounded-lg px-4 py-3 text-xs space-y-1">
              ...
              <div className="flex flex-wrap gap-1.5">
                {d.has_vomiting      && <span className="px-1.5 py-0.5 bg-red-100 dark:bg-red-900/40 text-slate-900 dark:text-slate-100 rounded font-semibold">Vómito</span>}
                {d.has_diarrhea      && <span className="px-1.5 py-0.5 bg-red-100 dark:bg-red-900/40 text-slate-900 dark:text-slate-100 rounded font-semibold">Diarrea</span>}
                {d.has_jaundice      && <span className="px-1.5 py-0.5 bg-red-100 dark:bg-red-900/40 text-slate-900 dark:text-slate-100 rounded font-semibold">Ictericia</span>}
                {d.has_sore_throat_fever && <span className="px-1.5 py-0.5 bg-red-100 dark:bg-red-900/40 text-slate-900 dark:text-slate-100 rounded font-semibold">Garganta/Fiebre</span>}
                {d.has_infected_wound && <span className="px-1.5 py-0.5 bg-red-100 dark:bg-red-900/40 text-slate-900 dark:text-slate-100 rounded font-semibold">Herida infectada</span>}
              </div>
            </div>
          ))}
        </div>
      ...
      {tableExists && (
        <>
          ...
          <div className="bg-white dark:bg-[#111827] rounded-xl border border-slate-100 dark:border-slate-700 p-5">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="bg-slate-100 dark:bg-slate-800 rounded-xl p-4 text-center">
                ...
                <p className="text-xs font-medium text-slate-600 dark:text-slate-300 mt-1">Declaraciones hoy</p>
              </div>
              <div className={`rounded-xl p-4 text-center ${symptomsToday > 0 ? "bg-white dark:bg-[#111827]" : "bg-slate-100 dark:bg-slate-800"}`}>
                <p className={`text-2xl font-black tabular-nums ${symptomsToday > 0 ? "text-amber-600" : "text-slate-600 dark:text-slate-300"}`}>{symptomsToday}</p>
                <p className="text-xs font-medium text-slate-600 dark:text-slate-300 mt-1">Con síntomas</p>
              </div>
              <div className={`rounded-xl p-4 text-center ${notClearedToday > 0 ? "bg-white dark:bg-[#111827]" : "bg-white dark:bg-[#111827]"}`}>
                <p className={`text-2xl font-black tabular-nums ${notClearedToday > 0 ? "text-red-600" : "text-green-600"}`}>{notClearedToday}</p>
                <p className="text-xs font-medium text-slate-600 dark:text-slate-300 mt-1">Excluidos hoy</p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { label: "Declaraciones (30d)",  value: total30,       icon: Users,       color: "text-rose-600",   bg: "bg-white dark:bg-[#111827]" },
              { label: "Exclusiones (30d)",    value: notCleared30,  icon: XCircle,     color: notCleared30 > 0 ? "text-red-600" : "text-slate-600 dark:text-slate-300",  bg: notCleared30 > 0 ? "bg-white dark:bg-[#111827]" : "bg-slate-100 dark:bg-slate-800" },
              { label: "Con síntomas (30d)",   value: declarations.filter((d) => !d.symptom_free).length, icon: AlertTriangle, color: declarations.filter((d) => !d.symptom_free).length > 0 ? "text-amber-600" : "text-slate-600 dark:text-slate-300", bg: declarations.filter((d) => !d.symptom_free).length > 0 ? "bg-white dark:bg-[#111827]" : "bg-slate-100 dark:bg-slate-800" },
              { label: "Hoy habilitados",      value: totalToday - notClearedToday, icon: ShieldCheck, color: "text-green-600",  bg: "bg-white dark:bg-[#111827]" },
            ].map((k) => (
          ...
          <div className="bg-white dark:bg-[#111827] border border-rose-200 dark:border-rose-800 rounded-xl px-5 py-4">
          ...
          <div className="bg-white dark:bg-[#111827] rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
              ...
              <span className="text-xs text-slate-600 dark:text-slate-300">{total30} registros</span>
            </div>
                  ...
                  <tbody className="divide-y divide-slate-50 dark:divide-slate-700/50">
                    {declarations.map((d) => (
                      <tr key={d.id} className={`hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors ${!d.cleared_to_work ? "bg-red-50/20 dark:bg-red-900/5" : ""}`}>
                        ...
                        <td className="px-4 py-3 text-xs text-slate-600 dark:text-slate-300">{SHIFT[d.shift] ?? d.shift}</td>
                        <td className="px-4 py-3 text-xs text-slate-600 dark:text-slate-300 tabular-nums">{d.declaration_time?.slice(0, 5) ?? "—"}</td>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex flex-wrap gap-1">
                            {d.has_vomiting      && <span className="text-[10px] px-1.5 py-0.5 bg-red-100 dark:bg-red-900/30 text-slate-900 dark:text-slate-100 rounded font-bold">Vómito</span>}
                            {d.has_diarrhea      && <span className="text-[10px] px-1.5 py-0.5 bg-red-100 dark:bg-red-900/30 text-slate-900 dark:text-slate-100 rounded font-bold">Diarrea</span>}
                            {d.has_jaundice      && <span className="text-[10px] px-1.5 py-0.5 bg-red-100 dark:bg-red-900/30 text-slate-900 dark:text-slate-100 rounded font-bold">Ictericia</span>}
                            {d.has_sore_throat_fever && <span className="text-[10px] px-1.5 py-0.5 bg-red-100 dark:bg-red-900/30 text-slate-900 dark:text-slate-100 rounded font-bold">Garganta/Fiebre</span>}
                            {d.has_infected_wound && <span className="text-[10px] px-1.5 py-0.5 bg-red-100 dark:bg-red-900/30 text-slate-900 dark:text-slate-100 rounded font-bold">Herida</span>}
                            {d.symptom_free && <span className="text-[10px] text-slate-600 dark:text-slate-300">Ninguno</span>}
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          {d.cleared_to_work
                        ...
                        <td className="px-4 py-3 text-xs text-slate-600 dark:text-slate-300 max-w-[180px] truncate">{d.restriction_note || "—"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
